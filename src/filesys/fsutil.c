...
fsutil_rm (char **argv) 
{
  ...
  printf ("Deleting '%s'...\n", file_name);
  if (!filesys_remove (dir_open_root (), file_name))
  ...
  for (;;)
    {
      ...
      if (type == USTAR_EOF)
      ...
      else if (type == USTAR_REGULAR)
        {
          ...
          if (!filesys_create (dir_open_root (), file_name, size, true))
            PANIC ("%s: create failed", file_name);
          dst = filesys_open (dir_open_root (), file_name);
          if (dst == NULL)
            PANIC ("%s: open failed", file_name);
...
fsutil_append (char **argv)
{
  ...
  /* Open source file. */
  src = filesys_open (dir_open_root (), file_name);
  if (src == NULL)
    PANIC ("%s: open failed", file_name);
